...
class FakeNewsClassifierModel(PreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig) -> None:
        ...
        self.bert2 = AutoModel.from_pretrained(config.bert_model_name)
        self.clf = nn.Sequential(
            nn.Linear(self.bert.config.dim*2, self.bert.config.dim*2),
            nn.ELU(),
            nn.Dropout(config.dropout_rate),
            nn.Linear(self.bert.config.dim*2, config.num_classes)
        )
...
training_args = TrainingArguments(
    output_dir='/gpfs/space/home/aral/mtProject/results/5',
    learning_rate=1e-5,
