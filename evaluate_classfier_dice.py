...
class EssayScorerModel(PreTrainedModel):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor,domain1_score) -> SequenceClassifierOutput:
        ...
        if domain1_score is not None:
            ...
            dLoss=dice_loss(logits, F.one_hot(domain1_score.long().to(logits.device),num_classes=self.num_labels))
            cLoss = loss_fn(logits.view(-1, self.num_labels), domain1_score.long().to(logits.device))
...
tokenizer = AutoTokenizer.from_pretrained('/gpfs/space/home/aral/mtProject/results/distilbert-smarttrunc-classifier-dice/checkpoint-62000-best')
